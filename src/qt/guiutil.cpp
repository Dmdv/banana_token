...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("banana_token"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("banana_token://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "banana_token:");
    }
...
{
    QString ret = QString("banana_token:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "banana_token.desktop";
    return GetAutostartDir() / strprintf("banana_token-%s.lnk", chain);
}
