Litecoin Core version 0.13.2 is now available from:
  ...
  <https://download.banana_token.org/banana_token-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a banana_token
...
This allows running banana_tokend without having to do any manual configuration.
...
`banana_token.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /banana_tokenrpc>
        ...
        # optional bypass banana_tokend rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< banana_tokenrpc:<password>
    </Location>
...
Restarting banana_tokend will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in banana_token.conf
...
banana_token-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/banana_token-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `banana_token-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `banana_token-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of banana_tokens they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of banana_tokens being spent is exactly
...
- `banana_token-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `banana_token-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
