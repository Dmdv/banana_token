...
- [Building Litecoin Core](#building-banana_token-core)
...
Clone the git repositories for banana_token and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/banana_token-project/banana_token
...
in the banana_token repository under 'Fetch and create inputs' to install sources which require
...
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the banana_token repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/banana_token/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/banana_token-project/banana_token
...
For example:
```bash
URL=https://github.com/thrasher-/banana_token.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit banana_token=${COMMIT} --url banana_token=${URL} ../banana_token/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit banana_token=${COMMIT} --url banana_token=${URL} ../banana_token/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit banana_token=${COMMIT} --url banana_token=${URL} ../banana_token/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the banana_token git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../banana_token/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/banana_token
SIGPATH=/some/root/path/banana_token-detached-sigs

./bin/gbuild --url banana_token=${BTCPATH},signature=${SIGPATH} ../banana_token/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/banana_token-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/banana_token-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/banana_token-osx-build.assert
```
...
[banana_token-project/gitian.sigs.ltc](https://github.com/banana_token-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
