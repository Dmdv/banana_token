...
example /usr/src/banana_token, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/banana_token-project/banana_token.git
    sudo chmod -R a+rw banana_token
...
way. This will install to `c:\workspace\banana_token`, for example:

    make install DESTDIR=/mnt/c/workspace/banana_token
...
causes two of the banana_token executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the banana_token code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the banana_token source code).
